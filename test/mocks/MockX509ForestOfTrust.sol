...
contract MockX509ForestOfTrust {
  ...
  constructor(bytes32 websiteNamehash, bytes32 authorityNamehash, bytes32 authorityCertId) public {
    // Trusted root cert
    certs[authorityCertId] = Certificate(0x1111111111111111111111111111111111111111, authorityCertId, 1, uint40(now - (94*24*60*60)), 5000000000, false, uint40(now - (93*24*60*60)), true, 24);
    toCertIds[authorityNamehash].push(authorityCertId);
    // Will make test fail if timestamp isn't checked
    certs[keccak256("\xdd\xdd\xdd")] = Certificate(0x5555555555555555555555555555555555555555, authorityCertId, 3, uint40(now - (94*24*60*60)), 5000000000, false, uint40(now - (92*24*60*60)), true, 320);
    toCertIds[websiteNamehash].push(keccak256("\xdd\xdd\xdd"));
    // The leaf cert that we're looking for
    certs[keccak256("\xbb\xbb\xbb")] = Certificate(msg.sender, authorityCertId, 2, uint40(now - 110000), 5000000000, false, uint40(now - 100000), true, 320);
    toCertIds[websiteNamehash].push(keccak256("\xbb\xbb\xbb"));
    certs[keccak256("\xcc\xcc\xcc")] = Certificate(0x5555555555555555555555555555555555555555, keccak256("\xcc\xcc\xcc"), 3, uint40(now - 110000), 5000000000, false, uint40(now - 100000), true, 320);
  ...
  function validNotBefore(bytes32 certId)
  external view returns (uint40)
  {
